...
public:
  ...
  auto dump_s() { return input_stream.rdbuf(); }
...
using custom_signal = boost::signals2::signal<void(std::string const)>;
...
protected:
  ...
  boost::optional<beast::flat_buffer> general_buffer_{};
  http::request<http::string_body> request_{};
  http::response<http::string_body> response_{};
  std::string current_number_{};
  std::string current_proxy_str_{};
  custom_signal signal_;
...
public:
  socks5_https_socket_base_t(net::ssl::context &, net::io_context &, Proxy &,
                             number_stream_t &, int);
  void start_connect();
  ~socks5_https_socket_base_t() { close_socket(); }
...
socks5_https_socket_base_t<Derived, Proxy>::socks5_https_socket_base_t(
    net::ssl::context &ssl_context, net::io_context &io_context,
    ...
    : io_{io_context}, ssl_stream_{std::in_place, net::make_strand(io_),
      ...
      ssl_context_{ssl_context}, scans_per_ip_{per_ip} {}
...
void socks5_https_socket_base_t<Derived, Proxy>::on_connected(
  ...
  spdlog::info("connected, performing SOCKS 5 handshake");
...
void socks5_https_socket_base_t<Derived, Proxy>::choose_next_proxy(
  ...
  current_proxy_str_ =
      boost::lexical_cast<std::string>(current_proxy_->endpoint_);
  if (!is_first_request) {
