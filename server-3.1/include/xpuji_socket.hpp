...
class xpuji_http_socket_t
    : public http_socket_base_t<xpuji_http_socket_t<Proxy>, Proxy> {

  using base_class = http_socket_base_t<xpuji_http_socket_t<Proxy>, Proxy>;
...
class xpuji_socks5_socket_t
    : public socks5_http_socket_base_t<xpuji_socks5_socket_t<Proxy>, Proxy> {
  using parent_class =
      socks5_http_socket_base_t<xpuji_socks5_socket_t<Proxy>, Proxy>;
...
template <typename Proxy>
void xpuji_http_socket_t<Proxy>::prepare_request_data(
...
template <typename Proxy>
void xpuji_http_socket_t<Proxy>::data_received(beast::error_code ec,
                                               std::size_t const) {
  if (ec) {
  ...
  static std::ofstream out_file{"./xpuji.txt"};
  if (out_file)
    out_file << body << "\n";
  /*
  json document;
    ...
    if (object.find("errorCode") != object.end()) {
      ...
      if (error_code == "0") {
        if (object.find("status") != object.end()) {
          ...
          if (status == "1") {
            signal_(search_result_type_e::Registered, this->current_number_);
          } else {
            signal_(search_result_type_e::Registered2, this->current_number_);
          }
        } else {
          signal_(search_result_type_e::Registered, this->current_number_);
        }
      } else if (error_code == "5") {
        signal_(search_result_type_e::NotRegistered, this->current_number_);
      } else {
        signal_(search_result_type_e::Unknown, this->current_number_);
      }
    } else {
      signal_(search_result_type_e::Unknown, this->current_number_);
    }
...
template <typename Proxy>
void xpuji_socks5_socket_t<Proxy>::prepare_request_data(
    bool use_authentication_header) {
  std::string const target =
...
template <typename Proxy>
void xpuji_socks5_socket_t<Proxy>::data_received(beast::error_code ec,
                                                 std::size_t const) {
  if (ec) {
    if (ec != beast::http::error::end_of_stream) {
      this->current_proxy_assign_prop(Proxy::Property::ProxyUnresponsive);
