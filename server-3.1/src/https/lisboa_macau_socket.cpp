...
std::string lisboa_macau_http_socket_t::hostname() const {
  return "yy99345.am";
}

void lisboa_macau_http_socket_t::prepare_request_data(
...
void lisboa_macau_http_socket_t::data_received(beast::error_code ec,
                                               std::size_t const) {
  if (ec) {
...
std::string lisboa_macau_socks5_socket_t::hostname() const {
  return "yy99345.am";
}

void lisboa_macau_socks5_socket_t::prepare_request_data(
    bool use_authentication_header) {
  using http::field;
  using http::verb;

  auto const target = "https://yy99345.am/Common/CheckData?DataType="
                      "telephone&DataContent=" +
                      current_number_ +
                      "&_=" + std::to_string(std::time(nullptr) * 1'000);
...
void lisboa_macau_socks5_socket_t::data_received(beast::error_code ec,
                                                 std::size_t const) {
