...
std::string grand_lisboa_http_socket_t::hostname() const {
  return "11333xpj.com";
}

void grand_lisboa_http_socket_t::prepare_request_data(
    bool use_authentication_header) {
  using http::field;

  auto target = "https://11333xpj.com:8787/Common/CheckData?DataType="
  ...
  request_.set(field::host, "11333xpj.com:8787");
  ...
  request_.set(field::referer, "https://11333xpj.com:8787/PageRegister?uid=");
  request_.set("X-Requested-With", "XMLHttpRequest");
  request_.body() = {};
  request_.prepare_payload();
}

void grand_lisboa_http_socket_t::data_received(beast::error_code ec,
                                               std::size_t const) {
  if (ec) {
...
std::string grand_lisboa_socks5_socket_t::hostname() const {
  return "11333xpj.com";
}

void grand_lisboa_socks5_socket_t::prepare_request_data(
    bool use_authentication_header) {
  using http::field;
  using http::verb;

  auto const target = "https://11333xpj.com:8787/Common/CheckData?"
                      "DataType=telephone&DataContent=" +
                      current_number_ +
                      "&_=" + std::to_string(std::time(nullptr) * 1'000);
...
void grand_lisboa_socks5_socket_t::data_received(beast::error_code ec,
                                                 std::size_t const) {
