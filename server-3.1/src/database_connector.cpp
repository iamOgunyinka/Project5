...
db_config_t parse_database_file(std::string const &filename,
  ...
  using utilities::splitStringView;
  using utilities::stringViewToString;
  ...
  while (std::getline(in_file, line)) {
    ...
    if (line[0] == '#' && line[1] == '~') { // new config
      ...
      auto name_pair = splitStringView(line, ":");
      if (name_pair.size() != 2)
        continue;

      std::string value = stringViewToString(name_pair[1]);
...
bool database_connector_t::add_upload(upload_request_t const &upload_request) {
  using utilities::boostViewToStdStringView;
  std::string const sql_statement{
      "insert into tb_uploads (uploader_id, filename, upload_date, status,"
      "total_numbers, name_on_disk) VALUES( {}, \"{}\", \"{}\", 0, {}, "
      "\"{}\" )"_format(
          boostViewToStdStringView(upload_request.uploader_id),
          boostViewToStdStringView(upload_request.upload_filename),
          boostViewToStdStringView(upload_request.upload_date),
          upload_request.total_numbers,
          boostViewToStdStringView(upload_request.name_on_disk))};
  try {
    {
      std::lock_guard<std::mutex> lock_g{db_mutex_};
...
bool database_connector_t::add_task(scheduled_task_t &task) {
  ...
  using utilities::unixTimeToString;

  std::string time_str{};
  std::size_t const count = unixTimeToString(time_str, task.scheduled_dt);
  ...
  std::string sql_statement{
      ...
      "\"{}\", 0, {}, 0, 0, 0, {}, {}, 0)"_format(
          ...
          integerListToString(task.number_ids), task.total_numbers,
...
bool database_connector_t::update_task_progress(
    internal_task_result_t const &task, std::size_t const total_ip_used) {
  std::string const sql_statement =
  ...
  if (task_ids.empty()) {
    ...
    sql_statement =
        ...
        "FROM tb_tasks WHERE scheduler_id ={} AND id IN ({})"_format(
            user_id.to_string(), utilities::stringListToString(task_ids));
  }
  std::lock_guard<std::mutex> lock_g{db_mutex_};
...
bool database_connector_t::set_input_files(std::string input_filename,
  ...
  replaceSpecialChars(input_filename);
  replaceSpecialChars(ok_filename);
  replaceSpecialChars(not_ok_filename);
...
void database_connector_t::delete_stopped_tasks(
  ...
  std::string const sql_statement =
      "DELETE FROM tb_tasks WHERE id in ({})"_format(
          utilities::integerListToString(task_ids));
  try {
    otl_cursor::direct_exec(otl_connector_, sql_statement.c_str(),
                            otl_exception::enabled);
  } catch (otl_exception const &e) {
...
bool database_connector_t::remove_filtered_tasks(
  ...
  std::string const sql_statement =
      ...
      "({})"_format(user_id.to_string(), utilities::integerListToString(ids));
  try {
    std::lock_guard<std::mutex> lock_g{db_mutex_};
    otl_cursor::direct_exec(otl_connector_, sql_statement.c_str(),
                            otl_exception::enabled);
