...
void digit_casinos_http_base_t::prepare_request_data(
  ...
  std::string fully_qualified_name{fqn()};
  auto const target =
  ...
  request_.set(field::referer, fully_qualified_name + "/PageRegister?uid=");
  request_.set("X-Requested-With", "XMLHttpRequest");
  request_.body() = {};
  request_.prepare_payload();
}

void digit_casinos_http_base_t::data_received(beast::error_code ec,
                                              std::size_t const) {
  if (ec) {
